...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/inspections/inspectionsSlice';
...
const TableSampleInspections = ({
  ...
  useEffect(() => {
    if (inspectionsNotify.showNotification) {
      notify(
        inspectionsNotify.typeNotification,
        inspectionsNotify.textNotification,
      );
    }
  ...
  const handleViewAction = (id: string) => {
    router.push(`/inspections/inspections-view/?id=${id}`);
  };

  const handleCreateEventAction = ({ start, end }: SlotInfo) => {
    router.push(
      `/inspections/inspections-new?dateRangeStart=${start.toISOString()}&dateRangeEnd=${end.toISOString()}`,
    );
  };
  ...
  return (
    <>
      ...
      {!showGrid && (
        <BigCalendar
          events={inspections}
          showField={'scheduled_date'}
          start-data-key={'scheduled_date'}
          end-data-key={'completion_date'}
          handleViewAction={handleViewAction}
          handleEditAction={handleEditAction}
          handleDeleteAction={handleDeleteModalAction}
          pathEdit={`/inspections/inspections-edit/?id=`}
          pathView={`/inspections/inspections-view/?id=`}
